...
def pmos_lvt_cmirror_netlist(
        ...
        multipliers: int
         ...
         netlist = Netlist(circuit_name='PMOS_lvt_current_mirror', nodes=['VCC', 'Iin', 'Iout', 'VG'])
         ...
         netlist.connect_netlist(fet_1.info['netlist'], [('D', 'Iin'), ('G', 'VG'), ('S', 'VCC'), ('B', 'VCC')]) 
         netlist.connect_netlist(fet_2.info['netlist'], [('D', 'Iout'), ('G', 'VG'), ('S', 'VCC'), ('B', 'VCC')]) 
...
def pmos_lvt_cmirror(     
    ...
    fet_A_1 = pmos_lvt(pdk, width=width, fingers=fingers, multipliers=multipliers, dummy=(True,False), length=length)
    fet_A_2 = pmos_lvt(pdk, width=width, fingers=fingers, multipliers=multipliers, dummy=(False,False), length=length)
    fet_B_1 = pmos_lvt(pdk, width=width, fingers=fingers, multipliers=multipliers, dummy=(False,False), length=length)
    fet_B_2 = pmos_lvt(pdk, width=width, fingers=fingers, multipliers=multipliers, dummy=(False,True), length=length)
    ...
    top_level << straight_route(pdk, fet_A_1_ref.ports["multiplier_0_source_E"], fet_B_1_ref.ports["multiplier_0_source_W"])
    top_level << c_route(pdk, fet_A_1_ref.ports["multiplier_0_drain_W"], fet_A_1_ref.ports["multiplier_0_gate_W"])
    ...
    gate_A2_via = top_level << viam2m3
    drain_A2_via = top_level << viam2m3
    drain_B1_via = top_level << viam2m3
    drain_B2_via = top_level << viam2m3

    gate_A2_via.move(fet_A_2_ref.ports["multiplier_0_gate_S"].center).movex(4.5)
    ...
    top_level << straight_route(pdk, fet_A_2_ref.ports["multiplier_0_drain_W"], drain_A2_via.ports["bottom_met_W"])
    top_level << c_route(pdk, fet_A_2_ref.ports["multiplier_0_gate_S"], gate_A2_via.ports["bottom_met_S"], e1glayer='met1', e2glayer='met3', cglayer='met2')
    top_level << c_route(pdk, gate_A2_via.ports["top_met_N"], drain_A2_via.ports["top_met_N"], cglayer='met2')
    ...
    top_level << c_route(pdk, drain_B1_via.ports["top_met_N"], drain_B2_via.ports["top_met_N"], extension=1, cglayer='met2')

    top_level.add_ports(gate_A2_via.get_ports_list(), prefix='A2_gate_')
    ...
    tap_encloses = (
            1.3 * (tap_separation + top_level.xmax),
    ...
    top_level << straight_route(pdk, fet_A_1_ref.ports["multiplier_0_dummy_L_gsdcon_top_met_W"], tapring_ref.ports["W_top_met_W"],glayer2="met2")
    top_level << straight_route(pdk, fet_B_2_ref.ports["multiplier_0_dummy_R_gsdcon_top_met_E"], tapring_ref.ports["E_top_met_E"],glayer2="met2")
    ...
    component.info['netlist'] = pmos_lvt_cmirror_netlist(pdk, width, length, fingers, multipliers)
