...
def n_fvf_cell_netlist(n_fvf: Component, c_mirr: Component) -> Netlist:
         ...
         netlist = Netlist(circuit_name='Voltage_Shifter_N_FVF_Cell ', nodes=['VCC', 'VIN', 'GND', 'VOUT', 'IBIAS', 'Ib', 'VG'])
         netlist.connect_netlist(n_fvf.info['netlist'], [('VIN', 'VIN'), ('AC_GND', 'GND'), ('VOUT', 'VOUT'), ('Ib', 'Ib')])
         netlist.connect_netlist(c_mirr.info['netlist'], [('VCC', 'VCC'), ('Iin', 'IBIAS'), ('Iout', 'Ib'), ('VG', 'VG')])

         return netlist

def  n_fvf_cell(
        pdk: MappedPDK,
        width: tuple[float,[float,float]] = (3,(3,3)),
        length: tuple[float,[float,float]] = (None,(None,None)),
    ...
    n_fvf = flipped_voltage_follower(pdk, width=width[1], length=length[1], fingers=fingers[1], multipliers=multipliers[1], dummy_1=dummy_1, dummy_2=dummy_2)
    ...
    top_level << straight_route(pdk, c_mirr_ref.ports["B_2_multiplier_0_source_E"], c_mirr_ref.ports["tie_E_top_met_E"])
